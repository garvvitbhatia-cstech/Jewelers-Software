...
class CategoriesTable extends Table{
	...
	public function validationUpdate($validator){
		$validator
			->notEmptyString('name', __('Please enter category.'))			
			->add('name', 'checkUniqueName', [
	...
	public function isCheckUniqueName($field,$id=NULL){
		if(isset($id['data']['edit_token']) && !empty($id['data']['edit_token'])){
			$data = $this->find()->where(array('name' => $field, 'id != ' => $this->decryptData($id['data']['edit_token'])))->first();
			if(isset($data->id)){
				return false;
			}else{
				return true;
			...
			$data = $this->find()->where(array('name' => $field))->first();
			if(isset($data->id)){
				return false;	
			}else{
				return true;

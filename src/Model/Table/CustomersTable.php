...
class CustomersTable extends Table{
	...
	public function validationAdd($validator){
		$validator
			...
			->notEmptyString('contact', __('Please enter your phone number.'))	
			
			->add('contact', 'uniquePhone', [
				'rule' => ['validateUnique'],
	...
	public function isCheckUniquePhone($field,$id=NULL){
		if(isset($id['data']['edit_token']) && !empty($id['data']['edit_token'])){
			$data = $this->find()->where(array('contact' => $field, 'id != ' => $this->decryptData($id['data']['edit_token'])))->first();
			if(isset($data->id)){
				return false;
			}else{
				return true;	
			...
			$data = $this->find()->where(array('contact' => $field))->first();
			if(isset($data->id)){
				return false;	
			}else{
				return true;	

...
class BarcodeController extends AppController{
    ...
    public function barcode(){
		...
		if($this->request->is(['Ajax','post'])){
			...
			$text = ($postData['text']!=""?$postData['text']:"0");
			$size = "20";
			$orientation = "horizontal";
			$code_type = "code39";
			$print = true;
			$sizefactor = 1;
			return $this->generateBarCode($filepath, $text, $size, $orientation, $code_type, $print, $sizefactor);
		}
		exit;	
		
	}
	
	//public function generateBarCode($filepath="", $text="0", $size="20", $orientation="horizontal", $code_type="code128", $print=false, $SizeFactor=1){
		...
		if(in_array(strtolower($code_type), array("code128", "code128b"))){
			...
			for($X=1; $X<=strlen($text); $X+=2){
				if(isset($temp[$X]) && isset($temp[($X + 1)])){
					...
					for($Y = 0; $Y < count($temp1); $Y++)$code_string .= $temp1[$Y] . $temp2[$Y];
				}
			}
	...
	/**************************** pagination page ****************************/
	public function barcodesFilter(){
        #check User Auth
        $this->checkValidSession();
		$table = TableRegistry::get('Products');
		# Conditions...
	...
	public function generateProductBarcode(){
        ...
        if($this->request->is(['post','put'])){
			...
			return $this->redirect(ADMIN_FOLDER.'barcodes/');	
		}	
	}
